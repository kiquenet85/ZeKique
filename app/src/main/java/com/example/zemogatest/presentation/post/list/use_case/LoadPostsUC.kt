package com.example.zemogatest.presentation.post.list.use_case
...
class LoadPostsUC @Inject constructor(
    ...
    suspend fun execute(refreshFromNetwork: Boolean) = withContext(Dispatchers.Default) {
        ...
        postRepository.getAll(GetPostInfo(refreshFromNetwork))
            .map { list ->
                list.map {
                    val uiItem = PostUI(it.id, it.body, it.favorite, it.userId, unreadCounter > 0)
                    unreadCounter--
                    uiItem
                }
